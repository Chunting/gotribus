...
public class WishListDao {
	...
	public int add( WishList wishList ) throws Exception {
		...
		try {
			...
			WishList id = (WishList)session.createSQLQuery("select * from wish_list where resourceId = ?" +
					...
					setInteger(1, wishList.getUser().getUserId()).
	...
	public boolean isAddResource(String type, int resourceId,int userId){
			String sql = "select * from wish_list where userId = ? and resourceId = ? and type = ?";								
			Session session = TribusHibernateSessionFactory.currentSession();		
	try {		
		//List l = session.createSQLQuery(sql).addEntity(MyTribusList.class).setInteger(0, userId).setInteger(1, resourceId).list();
		WishList l = (WishList)session.
					createSQLQuery(sql).
					addEntity(WishList.class).
					setInteger(0, userId).
					setInteger(1, resourceId).setString(2, type).
					uniqueResult();
	...
	public List<WishList> getWishListByUserId(int id){
		String sql = "select w.* from wish_list w,user_account u where w.userId = u.userId " +				
				" and u.userId = ? order by w.id desc ";				
