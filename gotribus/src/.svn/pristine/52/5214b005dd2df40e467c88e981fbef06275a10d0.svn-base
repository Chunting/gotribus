...
public class TribusMailController {
	...
	public String sendMailAction(HttpServletRequest request, HttpServletResponse res){
	    ...
	    public ModelAndView emailList(	    			    		
	    	...
	    	User user = getUser(request,res);	 
		 	if( user!= null){
				...
				PageObject<MessageVO> po = p.getResult(GlobleConfig.show_local+"userMail/box/"+typeId+".action",page < 0 ? 0 : page);
	 ...
	 public ModelAndView searchMessage(
		 ...
		 ModelAndView mv = new ModelAndView("user/maillist");
		 String nameOrContent = request.getParameter("search");
		 MessageDao md = new MessageDao();
		 HttpSession se = request.getSession();
	     User user = (User)se.getAttribute("user");
	     List<MessageVO> l = null;
		 try {
			l = md.searchAllMail(nameOrContent, user.getUserId());
