...
public class UserDao { 
	...
	public User login(String email, String pwd){
		...
		try {
			...
			u = (User)session.createQuery("from User where userEmail = :email and userPw = :pw")
			.setString("email", email).setString("pw", pwd).uniqueResult();																	
			return u;
	...
	public User haveRegistered(String email, String pwd){
		...
		try {
			...
			u = (User)session.createQuery("from User where userEmail = :email ").setString("email", email).uniqueResult();																	
			return u;
	...
	public User getUserByCondition(User u){
		...
		try {			
			...
			if(u.getUserVerifycode() != 0){c.add(Restrictions.eq("userVerifycode", u.getUserVerifycode()));}
...
//	}
	...
	public List<User> getUserListByCondition(User u){
		...
		try {			
			...
			Criteria c = session.createCriteria(User.class);
			if(u.getUserAlias() !=null){c.add(Restrictions.like("userAlias", u.getUserAlias()));}
			if(u.getUserEmail() !=null){c.add(Restrictions.eq("userEmail", u.getUserEmail()));}
			if(u.getUserPw()!=null){c.add(Restrictions.eq("userPw", u.getUserPw()));}
			if(u.getUserPic() !=null){c.add(Restrictions.eq("userPic", u.getUserPic()));}
			if(u.getUserVerifycode() != 0){c.add(Restrictions.eq("userVerifycode", u.getUserVerifycode()));}
	...
	public List<Activity> getActivityById(int id){
		...
		try {			
			u = session.
			createSQLQuery("select distinct a.* from activity a, activity_going ag  " +
					"where  ag.activityId = a.activityId and " +
					" ag.userId = ? union "+
							"select a.* from user_account u, activity a where u.userId = a.userId and" +
							"  a.userId = ?")
...
//		ud.cuttingBookPic();
		User u = new User();
		u.setUserAlias("guanzhichao");
		//1348227449@facebook.com
		//u.setUserEmail("1348227449@facebook.com");
		//http://gotribus.com/user/registerConfirm.action?user=
		//u.setUserVerifycode(Integer.parseInt("596839482"));
		List<User> u1 = ud.getUserListByCondition(u);
		
		//User u11 = ud.haveRegistered("jeremygzc@126.com", "");
