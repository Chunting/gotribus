...
//		}
	...
	public List<MovieMarkVo> getUserMovieSimilar(int user1,int user2){
		...
		for (MovieMarkVo movieMarkVo : m1) {
			if(Tribuscontains(m2,movieMarkVo)){
				l.add(movieMarkVo);
				i++;
			}
		}		
	...
	public List<MusicMarkVo> getUserMusicSimilar(int user1,int user2){
		...
		for (MusicMarkVo musicMarkVo : m1) {
			if(Tribuscontains(m2,musicMarkVo)){
				i++;
	...
	public boolean Tribuscontains(List<BookMarkVo> m1 ,BookMarkVo m2){				
		for (BookMarkVo movieMarkVo : m1) {
			if(m2.compareTo(movieMarkVo) == 0){
				return true;		
			}
		}
		return false;
	...
	public boolean Tribuscontains(List<MusicMarkVo> m1 ,MusicMarkVo m2){				
		for (MusicMarkVo movieMarkVo : m1) {
			if(m2.compareTo(movieMarkVo) == 0){
				return true;				
			}
		}
	...
	public boolean Tribuscontains(List<MovieMarkVo> m1 ,MovieMarkVo m2){				
		for (MovieMarkVo movieMarkVo : m1) {
			if(m2.compareTo(movieMarkVo) == 0){
				return true;				
			}
